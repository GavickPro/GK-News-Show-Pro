	 ...
	 function art_image($i, $only_value = false) {
	 	...
	 	if($art_image != '') {
 			...
 			if($this->parent->config['article_image_pos'] == 'left' && $this->parent->config['article_image_order'] == 1) {
 				return apply_filters('gk_nsp_art_image', '<div class="gk-nsp-image-wrap"><a href="'.$art_url.'" title="'.esc_attr(strip_tags($art_title)).'" class="gk-image-link"><img src="'.$art_image.'" alt="" class="gk-nsp-image" '.$style.' /></a></div>');
 			} else {
 				return apply_filters('gk_nsp_art_image', '<a href="'.$art_url.'" title="'.esc_attr(strip_tags($art_title)).'" class="gk-responsive gk-image-link"><img src="'.$art_image.'" alt="" class="gk-nsp-image gk-responsive" '.$style.' /></a>');
 			}
	 	} else {
	 		return '';
	 	} 
